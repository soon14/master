$(function () {
    ...
    //下拉框
    JY.Dict.setSelect("selectOrgLevel", "orgLevel", 2, "请选择");
    JY.Dict.setSelect("selectPosLevel","posLevel",2, "请选择");
    loadOrgTree();
    //增加回车事件
    ...
    $("#authorityDiv button").bind('click', function () {
        ...
        if (auth == 'org') {
            ...
            JY.Ajax.doRequest("", jypath + '/backstage/org/role/orglistAuthorized', {
                id: orgId,
                layer: layer
            }, function (data) {
            ...
            JY.Ajax.doRequest(null, jypath + '/backstage/org/role/listAuthorized', {
                id: roleId,
                layer: layer
            }, function (data) {
                $.fn.zTree.init($("#authorityTree"), {
    ...
    $('#addBtn').on('click', function (e) {
        ...
        var orgId = $("#baseForm input[name$='orgId']").val();
        if (JY.Object.notNull(orgId)) {
            loadRoleOrgTree(null)
            JY.Model.edit("auDiv", "新增角色", function () {
                if (JY.Validate.form("auForm")) {
                    var that = $(this);
                    JY.Ajax.doRequest("auForm", jypath + '/backstage/org/role/add', null, function (data) {
                        that.dialog("close");
                        JY.Model.info(data.resMsg, function () {
                            search();
                        });
...
function showPreOrg() {
    if (preisShow) {
        ...
        $("#preContent").css({left: op.left + "px", top: op.top + p.heith + "px"}).slideDown("fast");
        preisShow = true;
    }
}
function clickPreOrg(e, treeId, treeNode) {
    ...
    var tree = $.fn.zTree.getZTreeObj("preTree"),
        node = tree.getSelectedNodes(), v = "", n = "";
    for (var i = 0, l = nodes.length; i < l; i++) {
        v += nodes[i].name + ",";//获取name值
        n += nodes[i].id + ",";	//获取id值
    }
    for (var i = 0, l = node.length; i < l; i++) {
        v += node[i].name + ",";//获取name值
        n += node[i].id + ",";	//获取id值
    }
...
function clickOrg(event, treeId, treeNode) {
    ...
    $("#baseForm input[name$='pageNum']").val("1");
    $("#baseForm input[name$='pageSize']").val("5");
    getbaseList();
}
...
function loadRoleOrgTree(id) {
    JY.Ajax.doRequest(null, jypath + '/backstage/org/role/getRoleOrgTree', {id: id}, function (data) {
        $.fn.zTree.init($("#roleOrgTree"), {
            check: {
                enable: true,
                chkDisabledInherit: true,
...
function cleanForm() {
    ...
    hidePreOrg()
    $("#auForm select[name$='posLevel']").val("");
}
...
function setOrgForm(data) {
    ...
    $("#auOrgForm select[name$='orgLevel']").val(JY.Object.notEmpty(l.orgLevel));
...
function setForm(data,type) {
    ...
    var l = data.obj;
    loadRoleOrgTree(JY.Object.notEmpty(l.id));
    ...
    $("#auForm input[name$='orgLevel']").val(JY.Object.notEmpty(l.orgLevel));
    $("#auForm textarea[name$='description']").val(JY.Object.notEmpty(l.description));//描述
    $("#auForm select[name$='posLevel']").val(JY.Object.notEmpty(l.posLevel));
    $("#auForm input[name$='orgId']").prop("value",l.orgId);
    $("#pre").val(l.orgName);
}
