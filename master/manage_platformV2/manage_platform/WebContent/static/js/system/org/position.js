$(function () {
	...
	JY.Dict.setSelect("selectPosLevel","posLevel",'1');
	loadOrgTree();
	loadPreOrgTree();
	//安排人员
...
function clickOrg(event,treeId,treeNode) {
	cleanRightDiv();
		...
		if(o=='o'){	//判断是否组织 		
            ...
            $("#arrangeAccBtn").removeClass("hide");
			$("#title3").html(title1);
			$("#title4").html(title2);
			getPosList(1);
		}
...
function cleanPosForm(){
	...
	$("#auPosForm select[name$='posLevel']").prop("value","");//职务级别
	hidePreOrg();
}
function setPosForm(data){
	...
	$("#auPosForm select[name$='posLevel']").prop("value",JY.Object.notEmpty(l.posLevel));
...
function posKey(id){
			JY.Ajax.doRequest("",jypath +'/backstage/org/position/getPosOrgTree',{id:id},function(data){
			         ...
			         JY.Model.edit("authorityDiv","职务组织授权",function(){
			        		var zTree = $.fn.zTree.getZTreeObj("authorityTree"),
		 					...
		 					var that =$(this);
		 					JY.Ajax.doRequest(null,jypath +'/backstage/org/position/savePosOrg',{id:id,aus:aus},function(data){
		 	 					 that.dialog("close");
		 ...
		 $("#basePosTable tbody").empty();
    		 ...
    		 if(results!=null&&results.length>0){			
        		 ...
        		 for(var i = 0;i<results.length;i++){
            		 ...
            		 html+="<td class='center'>"+JY.Object.notEmpty(l.name)+"</td>";
            		 //var permitBtn=[{name:"移除人员角色",btnFun:"delAccPos",icon:"icon-remove-sign color-red"}];
            		 //html+=JY.Tags.setFunction(l.loginName,permitBtn);
            		 html+="</tr>";		 
            	 } 
...
function delAccPos(id){
	JY.Model.confirm("确认移除人员职务吗？",function(){	
		JY.Ajax.doRequest(null,jypath+'/backstage/org/position/delAccPos',{loginName:id},function(data){
			JY.Model.info(data.resMsg,function(){getPosList(1);});
		});
	});	
}
