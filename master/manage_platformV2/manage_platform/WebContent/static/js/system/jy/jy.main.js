...
JY={Object:{notNull:function(obj) {//判断某对象不为空..返回true 否则 false
    ...
    Dict:{//ids 对应id值(多个逗号分隔).keys 对应key值(多个逗号分隔).type(可选)1.请选择，2.自定义数组。默认不填.dfstr (可选)自定义数组
        setSelect:function(ids,keys,type,dfstr){
            $.ajax({type:'POST',url:jypath+'/backstage/dataDict/getDictSelect',data:{ids:ids,keys:keys},dataType:'json',
                success:function(data,textStatus){
                    if(data.res==1){
                        var map=data.obj;var idss= ids.split(",");var opts="",name="";
                        if(type==1){
                            ...
                            for(var i=0;i<idss.length;i++){
                                ...
                                $("#"+idss[i]+" label").html(name);
                                opts="";$.each(map[idss[i]].items,function(n,v) {opts+="<option value='"+v.value+"'>"+v.name+"</option>";});
                                $("#"+idss[i]+" select").append(opts);
                            }
                        }
                    }
    ...
    Page:{//跳转分页
        ...
        setPage:function(formId,pageId,pagesize,pagenum,totalCount,fun){
            if(totalCount>0){
                ...
                html+="<li class='disabled'><select onchange='JY.Page.setSize(&apos;"+formId+"&apos;,this.value,&apos;"+fun+"&apos;)' style='width:55px;float:left;' title='显示条数'>"+"<option value='10'  "+((pagesize==10)?"selected='selected'":"")+" >10</option>" +"<option value='20' "+((pagesize==20)?"selected='selected'":"")+" >20</option>" +"<option value='50' "+((pagesize==50)?"selected='selected'":"")+" >50</option>"+"</li>"+"<option value='100' "+((pagesize==100)?"selected='selected'":"")+" >100</option>"+"</li>";
                pageul.append(html);
            }
        },
        setPages:function(formId,pageId,pagesize,pagenum,totalCount,fun){
            if(totalCount>0){
                var pageul = $("#"+pageId+" ul"),html="";
                pageul.empty();
                var leng = parseInt((totalCount - 1)/pagesize)+1;
                ...
                if(end > leng){end = leng;start = leng > all ? (leng - all) : 1;}
                //现在设2,和显示对应
                html+="<li ><a class='btn btn-mini btn-success' href='##'>显示条数</a></li>";
                html+="<li class='disabled'><select onchange='JY.Page.setSize(&apos;"+formId+"&apos;,this.value,&apos;"+fun+"&apos;)' style='width:55px;float:left;' title='显示条数'>"+"<option value='10'  "+((pagesize==10)?"selected='selected'":"")+" >10</option>" +"<option value='20' "+((pagesize==20)?"selected='selected'":"")+" >20</option>" +"<option value='30' "+((pagesize==30)?"selected='selected'":"")+" >30</option>"+"</li>";
                pageul.append(html);
            }
        },
    ...
    Tags:{//设置按钮用的方法,id 这行的id,pBtn 按钮组
        ...
        setFunctionimport:function(name,type,pBtn){
            var h="";
            if(pBtn!=null&&pBtn.length>0){
                h+="<td class='center'>";
                h+="<div class='visible-md visible-lg hidden-sm hidden-xs btn-group'>";
                for(var i=0;i<pBtn.length;i++){h+="<a href='#' title='"+JY.Object.notEmpty(pBtn[i].name)+"' onclick='"+JY.Object.notEmpty(pBtn[i].btnFun)+"(&apos;"+name+","+type+"&apos;)' class='aBtnNoTD' ><i class='"+pBtn[i].icon+" bigger-140'></i></a>";}
                h+="</div>";
                h+="<div class='visible-xs visible-sm hidden-md hidden-lg'><div class='inline position-relative'>";
                h+="<button class='btn btn-minier btn-primary dropdown-toggle' data-toggle='dropdown'><i class='icon-cog icon-only bigger-110'></i></button>";
                h+="<ul class='dropdown-menu dropdown-only-icon dropdown-yellow pull-right dropdown-caret dropdown-close'>";
                for(var i=0;i<pBtn.length;i++){h+="<li><a href='#' title='"+JY.Object.notEmpty(pBtn[i].name)+"' onclick='"+JY.Object.notEmpty(pBtn[i].btnFun)+"(&apos;"+name+","+type+"&apos;)' class='aBtnNoTD' ><i class='"+pBtn[i].icon+" bigger-140'></i></a></li>";}
                h+="</ul></div></div>";
                h+="</td>";
            }else{h+="<td></td>";}
            return h;
    ...
    Model:{//loading框
        ...
        check:function(id,title,fn){
            $("#"+id).removeClass('hide').dialog({resizable:false, width: 650,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"查看")+"</h4></div>",title_html:true,buttons:[{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(fn) == 'function'){fn.call(this);}}}]});
        },
        edit:function(id,title,savefn,cancelfn){
            $("#"+id).removeClass('hide').dialog({resizable:false,width: 650,modal:true,title:"<div class='widget-header'><h4 class='smaller'>"+(JY.Object.notNull(title)?title:"修改")+"</h4></div>",title_html:true,buttons:[{html: "<i class='icon-ok bigger-110'></i>&nbsp;保存","class":"btn btn-primary btn-xs",click:function(){if(typeof(savefn) == 'function'){savefn.call(this);}}},{html:"<i class='icon-remove bigger-110'></i>&nbsp;取消","class":"btn btn-xs",click:function(){$(this).dialog("close");if(typeof(cancelfn) == 'function'){cancelfn.call(this);}}}]});
        },
        //审核
    ...
    Date:{//时间格式化(默认),time 时间
        ...
        Defaults:function(time){return JY.Object.notNull(time)?(new Date(time).Format("yyyy-MM-dd")):" ";},
