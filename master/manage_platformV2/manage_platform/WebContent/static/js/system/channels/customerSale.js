...
function getbaseList(init){
	...
	JY.Ajax.doRequest("baseForm",jypath +'/backstage/salesVolume/customerSales',null,function(data){
		$("#baseTable tbody").empty();
		var obj=data.obj;
		var list=obj.list;
		var allConsume=obj.sumSales;
		...
		if(results!=null&&results.length>0){
			...
			for(var i = 0;i<results.length;i++){
				...
				html+="<td class='center '><a id='' href='javascript:findsaleList("+l.userId+")'>"+JY.Object.notEmpty(l.money)+"</a></td>";
				html+="</tr>";
			}
...
function findsaleList(userId){
	...
	var index = jeBox.open({
		...
		type:2,
		maskClose:true,
		content:"findSaleList?userId="+userId+"&bTime="+beginTime+"&eTime="+endTime,
		button: [ {name: '取消'},
		]
	});
}
