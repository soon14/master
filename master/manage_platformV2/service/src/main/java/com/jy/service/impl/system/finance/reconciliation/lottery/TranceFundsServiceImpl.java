...
public class TranceFundsServiceImpl extends BaseServiceImp<TranceFunds> implements TranceFundsService {
    ...
    private void   _installObj(String thisDate,TranceFunds report) {
        ...
        Double ticketMoney=_findTicketMoney(date2);
        report.setTicketMoney(ticketMoney);
        Double refund=_findRefund(date2);
        report.setRefund(refund);
        Double addTranceMoney=_findAddTrance(date2);
...
//差异=期末-（期初-出票-退款+新增）
    ...
    private Double _findAddTrance(String date) {
        Double money=dao.findAddTranceMoney(date);
        return CalculationUtils.fomatMoney(money);
    }

    private Double _findRefund(String date) {
        Double money=dao.findRefundMoney(date);
        return CalculationUtils.fomatMoney(money);
    }

    private Double _findTicketMoney(String date) {
        Double money=dao.findTicketMoneyBetweenDate(date);
        return CalculationUtils.fomatMoney(money);
    }
