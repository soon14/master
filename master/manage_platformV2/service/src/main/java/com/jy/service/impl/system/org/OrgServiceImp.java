...
public class OrgServiceImp extends BaseServiceImp<Org> implements OrgService {
    ...
    public List<Org> findOrgsById(List<String> orgIds) {
        return ((OrgDao) baseDao).findOrgsById(orgIds);
    }
     ...
     */
    @Override
    public List<String> findSubSectorOrgsById(List<String> orgIds) {
        List<Org> orgs = ((OrgDao) this.baseDao).findSubSectorOrgsById(orgIds);
        if ((orgs == null) || (orgs.isEmpty()) || (orgs.size() == 0)) { return null; }
