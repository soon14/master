 ...
 */
@Service("scoreTransRequestProcess")
public class UserScoreTransProcessImpl extends ApiRequestTxtProcessImpl implements TxtInterface
{
    ...
    @Value("${score.transaction}")
    private String ftpFileName;//下载文件的名称
    private static final Logger LOG = LoggerFactory.getLogger(UserScoreTransProcessImpl.class);
    ...
    public int saveFTPData(String filePath, String date) {
        int count = 0;
        try {
            count = this.service.saveTransTxT(filePath, date);
            LOG.info("入库行数：{}", count);
        } catch (Exception e) {
            LOG.info("用户积分明细接口保存数据失败！");
            e.printStackTrace();
        }
        return count;
    }
