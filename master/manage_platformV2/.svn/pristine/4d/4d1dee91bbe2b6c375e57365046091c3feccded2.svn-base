...
public class SalesManagerController extends BaseController<Object> {
			...
			try {
				...
				if(null!=list){
					for (int i = 0; i < list.size(); i++) {
						...
						if(list.get(i).getmCpUserId().equals("0")){
							list.get(i).setChildCustomerNum("0");
						}else{
						...
						String roleName = merchantService.findRoleName(mId);
		...
		if ((doSecurityIntercept(Const.RESOURCES_TYPE_FUNCTION))) {
			...
			try {
				...
				if(!isPhoneOrTel(merchant.getmMobile())){
					ar.setSucceedMsg("2");
					return ar;
				}else if(merchantService.findmMobile(merchant.getmMobile(),"")!=0){
					ar.setSucceedMsg("3");
					return ar;
				}

				merchant.setMParentMerchant("2");
	...
	public AjaxRes roleTree() {
		...
		try {
			if(userId.equals("2")){
				list = accountService.getRoles();
			}else{
				list = accountService.getRolesByRoleId(userId);
			}
			ar.setSucceed(list);
		} catch (Exception e) {
			logger.error(e.toString(), e);
