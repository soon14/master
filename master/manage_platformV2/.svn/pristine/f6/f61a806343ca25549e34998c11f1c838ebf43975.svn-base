...
function getbaseList(init){
	...
	JY.Model.loading();
	JY.Ajax.doRequest("baseForm",jypath +'/channels/secondaryAudit/findByPage',null,function(data){
		 $("#baseTable tbody").empty();
    		 ...
    		 if(results!=null&&results.length>0){
        		 ...
        		 for(var i = 0;i<results.length;i++){
					 ...
					 // if(l.bcFlag==1) html+="<td class='center hidden-480' >有</td>"
					 // else html+="<td class=\"center\">"+
						//  "<div class=\"visible-md visible-lg hidden-sm hidden-xs btn-group\">"+
						//  "<a href=\"#\" title=\"预付款\" onclick=\"prepayment(\'"+l.mId+"\',\'"+l.mName+"\')\"class=\"aBtnNoTD\">无</a>"+
						// 	 //<i class=\"icon-jpy color-blue bigger-140\"></i>
						//  "</div></td>";
...
//审核
function secondaryAuditThrough(accountId){
	JY.Model.confirm("确认通过吗？",function(){
		JY.Ajax.doRequest(null,jypath +'/channels/secondaryAudit/secondaryAuditThrough',{mId:accountId},function(data){
			JY.Model.info(data.resMsg,function(){search();});
		});
	});
}
...
function prepayment(mID,mName) {
	setPrepaymentForm(mID, mName);
	JY.Model.edit("prepaymentId", "预收款管理", function () {
...
/** 新增预收款配置*/
function setPrepaymentForm(mId,mName){
...
function setForm(data){
    ...
    if(l.mLicense!=null&&l.mLicense.length>0) {
        ...
        for (var j = 0; j < mLicense.length; j++) {
            hdfiles+="<a href='/merchant" + mLicense[j] + "' title='Photo Title' data-rel='colorbox' class='cboxElement'>"+
