...
function getbaseList(init){
	...
	JY.Ajax.doRequest("baseForm",jypath +'/backstage/salesVolume/findSvByPage',null,function(data){
		$("#baseTable tbody").empty();
		var obj=data.obj;
		var list=obj.list;
		var allConsume=obj.allConsume;
		var allCommission=obj.allCommission;
		if(allConsume==null){
			allConsume=0;
		}
		...
		$("#allConsume").text(allConsume);
		$("#allCommission").text(allCommission);
		// var voList=obj.voList;
		// if(voList.length!=0){
		// 	$("#count").text("子渠道数");
		// 	$("#name").text("部门");
		...
		// 		// html+="<td class='center '><a id='' href='javascript:orgMerchantList("+l.orgId+");'>"+JY.Object.notEmpty(l.orgName)+"</a></td>";
		...
		// 		html+="<td class='center hidden-480'>"+JY.Object.notEmpty(l.childMerchantCount)+"</td>";
		...
		if(results!=null&&results.length>0){
			...
			for(var i = 0;i<results.length;i++){
				...
				// html+="<td class='center hidden-480'>"+JY.Object.notEmpty(l.sales)+"</td>";
				html+="<td class='center '><a id='' href='javascript:doQuery("+l.merchantId+","+l.level+");'>"+JY.Object.notEmpty(l.sales)+"</a></td>";
				...
				html+="<td class='center hidden-480'>"+JY.Object.notEmpty(l.commission)+"</td>";
				//html+="<td class='center hidden-480''>"+JY.Date.Format(l.date,'yyyy-MM-dd hh:mm:ss')+"</td>";
				html+="<td class='center '>"+JY.Object.notEmpty(l.name)+"</td>";
				html+=JY.Tags.setFunction(l.merchantId,permitBtn);
				html+="</tr>";
			}
			$("#baseTable tbody").append(html);
...
function doQuery(merchantId,level){
	if(level==2){
		findchildMerchant(merchantId);
	}else{
		childMerchantAndSignList(merchantId)
	}
}
