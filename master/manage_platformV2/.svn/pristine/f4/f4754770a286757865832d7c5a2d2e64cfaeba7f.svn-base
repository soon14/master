    ...
    JY.Ajax.doRequest("baseForm", jypath + '/backstage/bettingDifference/findByPage', null, function (data) {
        ...
        if (results != null && results.length > 0) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                html += "<td class='center ' ><a id='' onclick='saleNumberDifference(" + l.id + ",\"" + l.statisticsDate + "\"," + 1 + ")'>" + JY.Object.notEmpty(l.numberDifference) + "</a></td>";
                html += "<td class='center '><a id='' onclick='saleNumberDifference(" + l.id + ",\"" + l.statisticsDate + "\"," + 2 + ")'>" + JY.Object.notEmpty(l.amountDifference) + "</a></td>";
                html += "</tr>";
            }
...
function saleNumberDifference(ids, date, n) {
    var number = n;
    var builDate = date;
    var index = jeBox.open({
        ...
        type: 2,
        maskClose: true,
        content: "buillIndex?billDate=" + builDate + "&ids=" + ids + "&number=" + number,
        button: [{name: '取消'},
        ],
    });
}
