   ...
   private OutLineDataProcess outLineDataProcess;
    ...
    public String addOutLineDataInit(Model model){
        ...
        if(list.size()>0){
            ...
            if(deposit>0){
                String strVal= df.format(deposit);
                model.addAttribute("addExist","1");
                model.addAttribute("deposit",strVal);
            }
        }
        model.addAttribute("permitBtn", getPermitBtn(Const.RESOURCES_TYPE_FUNCTION));
    ...
    @ResponseBody
    public AjaxRes addDeposit(Merchant merchant){
        AjaxRes ar = getAjaxRes();
        try {
    ...
    @ResponseBody
    public AjaxRes delDeposit(Merchant merchant, HttpServletRequest request){
        String id = request.getParameter("id");
        Map map = new HashMap();
        map.put("id",id);
        AjaxRes ar = getAjaxRes();
        try {
                outLineDataProcess.delDeposit(map);
            ar.setSucceedMsg("success");
        } catch (Exception e) {
            e.printStackTrace();ar.setSucceedMsg("fail");
        }
    ...
    @ResponseBody
    public AjaxRes updateDeposit(Merchant merchant, HttpServletResponse response){
        AjaxRes ar = getAjaxRes();
        try {
                outLineDataProcess.updateDeposit(merchant);
            ar.setSucceedMsg("success");
        } catch (Exception e) {
            e.printStackTrace();ar.setSucceedMsg("fail");
        }
