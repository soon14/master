...
public class FtpUtil {
    ...
    public static Map<String, String> downloadAndChekData(String ftpPath, String ftpFile, String localPath, FtpClient ftp) {
        ...
        try {
            ...
            if (maxNo != 0) {
                ...
                mapReturn.put("notFile", TxtDataEnum.NOT_FILE.getViewName());
                return mapReturn;
            }
            //得到ftp文件
        ...
        if ((diff == 0 && null == checkResult.get(CHECK_KEY)) || Integer.parseInt(rowsNumber) == 0) {
            ...
            if (null != localServerPath) {
                ...
                LOG.error("#未生成{}文件！", localServerPath);
                Map<String, String> mapReturn = new HashMap<>();
                mapReturn.put("notBakFile", TxtDataEnum.NOT_GENERATED_BAK.getViewName());
                return mapReturn;
            }
        } else {
            LOG.info("Unavailable file!!");
            if (diff != 0) {
                String value = "文件名为：" + (ftpFile + maxNo) + "，行数不一致，FTP文件的行数为" + rowsNumber + "，经检查过的行数为" + (i - 3);
                checkResult.put(CHECK_KEY, value);
            }
            writeFile(sblocalFile.toString(), false, checkResult.get(CHECK_KEY));
        }
        Map<String, String> mapReturn = new HashMap<>();
        mapReturn.put("notErrFile", TxtDataEnum.GENERATED_ERR.getViewName());
        return mapReturn;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(ftp.getWelcomeMsg());
        Map<String, String> path = FtpUtil.downloadAndChekData("/daysluck/20170623/", "DSCS_QUL_20170623_", "E:/localhost/", ftp);
