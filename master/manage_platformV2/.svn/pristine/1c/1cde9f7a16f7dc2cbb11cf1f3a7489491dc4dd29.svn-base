...
import com.jy.service.impl.system.finance.reconciliation.game.GameUserPaymentInfoServiceImpl;
...
/** 
 * @文件名:GameUserPaymentInfoTask.java 
 * @功能描述：
 * @创建日期:2017年4月1日下午12:00:28 
 * @创建人：Dingj
 * @Copyright（c） 2017,all rights reserved by days 
 */
public class GameUserPaymentInfoTask implements Job {
	 /* 日志对象 */
    ...
    private void synchronousData() throws IOException {
        log.info(DateUtils.getDate("yyyy-MM-dd HH:mm:ss")+" ——同步用户充值数据任务开始运行");
        ApplicationContext ac = SpringWebContextUtil.getApplicationContext();
        GameUserPaymentInfoServiceImpl service  = (GameUserPaymentInfoServiceImpl) ac.getBean("gameUserPaymentInfoService");
        service.switchData();
        log.info(DateUtils.getDate("yyyy-MM-dd HH:mm:ss")+" ——同步用户充值数据任务结束");
}
}
