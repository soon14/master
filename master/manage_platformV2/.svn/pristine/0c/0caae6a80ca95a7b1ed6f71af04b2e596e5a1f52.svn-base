 ...
 */
@Controller
@RequestMapping("/statistics/qrCodeDailyReport/")
public class QrCodeDailyReportController extends BaseController<QrCodeDailyReport> {

    @Autowired
    private DailyProcess process;
    @Autowired
    ...
    public String index(Model model){
        if(doSecurityIntercept(Const.RESOURCES_TYPE_MENU)){
            ...
            return "/system/finance/statistics/qrcode/QrCodeDailyReport";
        }
    ...
    @ResponseBody
    public AjaxRes findByPage(Page<QrCodeDailyVo> page, BaseForm form){
        AjaxRes ar=getAjaxRes();
        if(ar.setNoAuth(doSecurityIntercept(Const.RESOURCES_TYPE_MENU,"/statistics/qrCodeDailyReport/index"))){
            try {
                formProcess.check(form);
                Page<QrCodeDailyVo> reports=process.findByPage( page,form);
    ...
    public String resetData(Model model){
        if(doSecurityIntercept(Const.RESOURCES_TYPE_BUTTON)){
            ...
            return "/system/finance/statistics/qrcode/dailyResetData";
        }
        return Const.NO_AUTHORIZED_URL;
    }
    ...
    public AjaxRes manual(BaseForm form){
        ...
        if(ar.setNoAuth(doSecurityIntercept(Const.RESOURCES_TYPE_BUTTON,"/statistics/qrCodeDailyReport/resetData"))){
            try {
                manualProcess.manualByType(form,4);
                Map<String, Object> p=new HashMap<String, Object>();
                p.put("msg", "数据重置成功!");
                ar.setSucceed(p);
            }catch (Exception e){
