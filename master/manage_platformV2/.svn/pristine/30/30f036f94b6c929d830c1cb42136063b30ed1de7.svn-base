...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
     ...
     * @return
     */
	public static List<String> findDatesFormat(String dBegin, String dEnd) {
		List<String> lDate = new ArrayList();
		// lDate.add(dBegin);
		Calendar calBegin = Calendar.getInstance();
		// 使用给定的 Date 设置此 Calendar 的时间
		calBegin.setTime(DateUtils.stringToDate(dBegin,"yyyyMMdd"));
		Calendar calEnd = Calendar.getInstance();
		// 使用给定的 Date 设置此 Calendar 的时间
		Date dateEnd = DateUtils.stringToDate(dEnd,"yyyyMMdd");
		calEnd.setTime(dateEnd);
		// 测试此日期是否在指定日期之后
		while (dateEnd.after(calBegin.getTime()) || dateEnd.equals(calBegin.getTime())) {
			lDate.add(DateUtils.format(calBegin.getTime(),"yyyyMMdd"));// 根据日历的规则，为给定的日历字段添加或减去指定的时间量
			calBegin.add(Calendar.DAY_OF_MONTH, 1);
		}
		return lDate;
	}
